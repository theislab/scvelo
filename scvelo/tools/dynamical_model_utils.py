...
def convolve(x, weights=None):
    return (
        (weights.multiply(x).tocsr() if issparse(weights) else weights * x)
    ...
    if mode == "tau":
        ...
        if clusters is not None:
            ...
            for cat in clusters.cat.categories:
                ...
                n = np.clip(
                    np.sum(np.invert(np.isnan(distx_cat)), 0) - len(distx_cat) * 0.01, 2, None
                )
...
def curve_dists(
    u, s, alpha, beta, gamma, t_=None, u0_=None, s0_=None, std_u=1, std_s=1, scaling=1, num=None
):
    if u0_ is None or s0_ is None:
        u0_, s0_ = mRNA(t_, 0, 0, alpha, beta, gamma)
...
class BaseDynamics:
    def __init__(
        ...
        self.alpha, self.beta, self.gamma, self.scaling, self.t_, self.alpha_ = (
            None,
            None,
            None,
            None,
        ...
        self.u0_, self.s0_, self.weights, self.weights_outer, self.weights_upper, = (
            None,
            None,
            None,
            None,
        ...
        self.t, self.tau, self.o, self.tau_, self.likelihood, self.loss, self.pars = (
            None,
            None,
            None,
            None,
    ...
    def load_pars(self, adata, gene):
        ...
        t = (
            adata.obs["shared_time"]
            if "shared_time" in adata.obs.keys()
            else adata.layers["fit_t"][:, idx]
        )
        ...
        if isinstance(self.refit_time, bool):
            ...
            self.t = (
                adata.obs[self.refit_time].values
                if isinstance(self.refit_time, str)
                else self.refit_time
            )
    ...
    def get_vars(
        self, alpha=None, beta=None, gamma=None, scaling=None, t_=None, u0_=None, s0_=None
    ):
    ...
    def get_dists(
        ...
        if reg is None:
            reg = (
                (gamma / beta - self.steady_state_ratio) * s / self.std_s
    ...
    def get_loglikelihood(self, varx=None, noise_model="normal", **kwargs):
        ...
        if noise_model == "normal":
            ...
            loglik = -1 / np.sqrt(2) / n * np.sum(eucl_distx) / np.sqrt(varx) - 1 / 2 * np.log(
                2 * varx
            )
        else:
    ...
    def get_curve_likelihood(self):
        ...
        dist, dist_ = curve_dists(
            u, s, alpha, beta, gamma, t_, std_u=self.std_u, std_s=self.std_s, scaling=scaling
        )
    ...
    def plot_phase(
        ...
        **kwargs,
        ...
        if show_assignments:
            ax.plot(
                np.array([self.s[idx_sorted], st]),
                np.array([self.u[idx_sorted], ut]),
                color="grey",
                linewidth=0.1 * 1,
            )
    ...
    def plot_profile_contour(
        ...
        **kwargs,
        ...
        if contour_levels != 0:
            contours = ax.contour(x, y, log_zp, levels=contour_levels, colors="k", linewidths=0.5)
    ...
    def plot_state_likelihoods(
        ...
        **kwargs,
        ...
        if color_map is None:
            color_map = rgb_custom_colormap(["royalblue", "white", "seagreen"], alpha=[1, 0.5, 1])
        if color_map_steady is None:
            color_map_steady = rgb_custom_colormap(colors=3 * ["sienna"], alpha=[0, 0.5, 1])
        ...
        ax.scatter(
            x=s, y=u, s=50, c=likelihoods, zorder=3, cmap=color_map, edgecolors="black", **kwargs
        )
        ...
        if continuous:
            ...
            contf_steady = ax.contourf(
                ss, uu, l_grid_steady, vmin=0, vmax=vmax_steady, levels=30, cmap=color_map_steady
            )
        ...
        if transitions is not None:
            ...
            transitions = np.multiply(
                np.array(transitions), [np.min(likelihoods_grid), np.max(likelihoods_grid)]
            )  # trans_width
            ax.contour(
                ss,
                uu,
    ...
    def get_pval(self, model="dynamical", **kwargs):
        ...
        if "weighted" not in kwargs:
            kwargs["weighted"] = "outer"
        distx = (
            self.get_orth_distx(**kwargs) if model == "orthogonal" else self.get_distx(**kwargs) / 2
        )
    ...
    def get_pval_diff_kinetics(self, orth_beta=None, min_cells=10, **kwargs):
        ...
        p-value
        """
        if "weights_cluster" in kwargs and np.sum(kwargs["weights_cluster"]) < min_cells:
            return 1
        if "weighted" not in kwargs:
            kwargs["weighted"] = "outer"
    ...
    def get_cluster_mse(self, clusters=None, min_cells=10, weighted="outer"):
        ...
        mse = np.array(
            [self.get_mse(weights_cluster=self.clusters == c, weighted=weighted) for c in self.cats]
        )
    ...
    def get_cluster_pvals(self, clusters=None, model=None, orth_beta=None, **kwargs):
        ...
        pvals = np.array(
            [
                ...
                if model is None
                else self.get_pval(model=model, weights_cluster=self.clusters == c, **kwargs)
                for c in self.cats
            ]
        )
        return pvals
    ...
    def differential_kinetic_test(self, clusters, as_df=None, min_cells=10, weighted="outer"):
        ...
        if as_df:
            return pd.DataFrame(
                np.array(round(self.pvals_kinetics, 2))[None, :], columns=self.cats, index=["pval"]
            )
...
def get_latent_vars(adata, scaled=True, key="fit"):
    ...
    pval_steady = (
        adata.var[f"{key}_pval_steady"].values if f"{key}_pval_steady" in adata.var.keys() else None
    )
    steady_u = (
        adata.var[f"{key}_steady_u"].values if f"{key}_steady_u" in adata.var.keys() else None
    )
    steady_s = (
        adata.var[f"{key}_steady_s"].values if f"{key}_steady_s" in adata.var.keys() else None
    )

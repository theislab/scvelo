...
class Velocity:
    def __init__(
        ...
        self._Mu = (
            adata.layers["unspliced"] if use_raw else adata.layers["Mu"] if Mu is None else Mu
        )
        ...
        self._offset, self._offset2 = (
            np.zeros(n_vars, dtype=np.float32),
            np.zeros(n_vars, dtype=np.float32),
        )
        self._gamma, self._r2 = (
            np.zeros(n_vars, dtype=np.float32),
            np.zeros(n_vars, dtype=np.float32),
        )
        self._beta, self._velocity_genes = (
    ...
    def compute_stochastic(self, fit_offset=False, fit_offset2=False, mode=None, perc=None):
        ...
        idx = (
            ...
            else np.ones(self._velocity_genes.shape, dtype=bool)
        )
        ...
        self._offset[idx], self._offset2[idx], self._gamma[idx] = (
            ...
            if mode == "bayes"
                ...
                _Ms, _Mu, var_ss, cov_us, res_std, res2_std, fit_offset, fit_offset2, perc
            )
        )
...
def velocity(
    ...
    **kwargs,
    ...
    if mode is None or (mode == "dynamical" and "fit_alpha" not in adata.var.keys()):
        mode = "stochastic"
        logg.warn(
            "Falling back to stochastic model. For the dynamical model run tl.recover_dynamics first."
        )
    ...
    if mode in {"dynamical", "dynamical_residuals"}:
        ...
        raise ValueError(
            "Mode can only be one of these: steady_state, deterministic, stochastic or dynamical."
        )

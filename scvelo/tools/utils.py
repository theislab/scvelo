...
def prod_sum_obs(A, B):
    ...
    """
    return (
        A.multiply(B).sum(0).A1
        if issparse(A)
        else np.einsum("ij, ij -> j", A, B)
        if A.ndim > 1
        else (A * B).sum()
    )
...
def prod_sum_var(A, B):
    ...
    """
    return (
        A.multiply(B).sum(1).A1
        if issparse(A)
        else np.einsum("ij, ij -> i", A, B)
        if A.ndim > 1
        else (A * B).sum()
    )
...
def norm(A):
    ...
    """
    return np.sqrt(
        A.multiply(A).sum(1).A1
        if issparse(A)
        else np.einsum("ij, ij -> i", A, A)
        if A.ndim > 1
        else np.sum(A * A)
    )
...
def R_squared(residual, total):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        r2 = np.ones(residual.shape[1]) - prod_sum_obs(residual, residual) / prod_sum_obs(
            total, total
        )
    r2[np.isnan(r2)] = 0
    return r2
...
def cosine_correlation(dX, Vi):
    ...
    with warnings.catch_warnings():
        ...
        result = (
            np.zeros(dx.shape[0])
            if Vi_norm == 0
            else np.einsum("ij, j", dx, Vi) / (norm(dx) * Vi_norm)[None, :]
        )
    return result
...
def normalize(X):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        X = X.multiply(csr_matrix(1.0 / np.abs(X).sum(1))) if issparse(X) else X / X.sum(1)
    return X
...
def geometric_matrix_sum(C, n_power=2):  # computes C + C^2 + C^3 + ...
    C_n = geometric_matrix_sum(C, n_power - 1) if n_power > 2 else C if n_power > 1 else 0
    return C + C.dot(C_n)

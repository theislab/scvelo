...
def get_mean_var(X, ignore_zeros=False, perc=None):
    ...
    mean_sq = (
        (X.multiply(X).sum(0) / n_counts).A1 if issparse(X) else np.multiply(X, X).sum(0) / n_counts
    )
...
def rank_velocity_genes(
    ...
    if "spearmans_score" not in adata.var.keys():
        corr = vcorrcoef(
            np.array(adata.layers["Ms"]).T, np.array(adata.layers["Mu"].T), mode="spearmans"
        )
    ...
    if "spearmans_score" in adata.var.keys():
        ...
        min_corr = 0.1 if min_corr is None else min_corr  # np.percentile(r2[r2 > 0], 50)
        tmp_filter &= corr > min_corr

    if "dispersions_norm" in adata.var.keys():
        dispersions = adata.var.dispersions_norm
        min_dispersion = (
            0 if min_dispersion is None else min_dispersion
        )  # np.percentile(dispersions, 20)
        tmp_filter &= dispersions > min_dispersion

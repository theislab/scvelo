...
def convert_to_adata(vlm, basis=None):
    ...
    X = (
        vlm.S_norm.T if hasattr(vlm, "S_norm") else vlm.S_sz.T if hasattr(vlm, "S_sz") else vlm.S.T
    )  # .sparse().T.tocsr()
...
def convert_to_loom(adata, basis=None):
    ...
    class VelocytoLoom(velocyto.VelocytoLoom):
        def __init__(self, adata, basis=None):
            ...
            self.S = (
                np.array(self.S.A, **kwargs) if issparse(self.S) else np.array(self.S, **kwargs)
            )
            self.U = (
                np.array(self.U.A, **kwargs) if issparse(self.U) else np.array(self.U, **kwargs)
            )
        ...
        def filter_and_normalize(
            ...
            self._normalize_S(
                relative_size=self.initial_cell_size, target_size=np.median(self.initial_cell_size)
            )
            self._normalize_U(
        ...
        def velocity_estimation(
            self, fit_offset=False, perc=[5, 95], filter_genes=False, limit_gamma=False
        ):
            self.fit_gammas(
        ...
        def run_all(
            ...
            self.filter_and_normalize(
                min_counts=min_counts, min_counts_u=min_counts_u, n_top_genes=n_top_genes
            )

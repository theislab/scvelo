...
class DynamicsRecovery(BaseDynamics):
    ...
    def initialize(self):
        ...
        weights_g = (
            weights_s if self.steady_state_prior is None else weights_s | self.steady_state_prior[w]
        )
        beta, gamma = (
            1,
            linreg(convolve(u_w, weights_g), convolve(s_w, weights_g)) + 1e-6,
        ...
        self.alpha, self.beta, self.gamma, self.scaling, self.alpha_, = (
            alpha,
            beta,
            gamma,
            scaling,
    ...
    def fit_t_and_alpha(self, **kwargs):
        ...
        res = minimize(
            mse,
            np.array([self.t_, self.alpha]),
            callback=self.cb_fit_t_and_alpha,
            **self.simplex_kwargs,
        )  # method='Nelder-Mead')
    ...
    def fit_rates(self, **kwargs):
        ...
        res = minimize(
            mse,
            np.array([self.alpha, self.gamma]),
            tol=1e-2,
            callback=self.cb_fit_rates,
            **self.simplex_kwargs,
        )
    ...
    def fit_rates_all(self, **kwargs):
        ...
        res = minimize(
            mse,
            np.array([self.alpha, self.beta, self.gamma]),
            tol=1e-2,
            callback=self.cb_fit_rates_all,
            **self.simplex_kwargs,
        )
    ...
    def fit_t_and_rates(self, **kwargs):
        ...
        res = minimize(
            mse,
            np.array([self.t_, self.alpha, self.beta, self.gamma]),
            tol=1e-2,
            callback=self.cb_fit_t_and_rates,
            **self.simplex_kwargs,
        )
    ...
    def fit_scaling_(self, **kwargs):
        ...
        res = minimize(
            mse,
            np.array([self.t_, self.beta, self.scaling]),
            callback=self.cb_fit_scaling_,
            **self.simplex_kwargs,
        )
    ...
    def update(
        ...
        if adjust_t_ and np.any(on):
            ...
            if 0 < alt_t_ < t_:
                ...
                alt_t_ += (
                    np.max(t) / len(t) * np.sum(t == t_)
                )  # np.sum((self.u / self.scaling >= alt_u0_) | (self.s >= alt_s0_))
                alt_t, alt_tau, alt_o = self.get_time_assignment(
                    alpha, beta, gamma, scaling, alt_t_
                )
                ...
                if alt_loss * 0.99 <= np.min([loss, loss_prev]) or ut_cur * 0.99 < ut_alt:
                    t, tau, o, t_, loss, perform_update = (
                        alt_t,
                        alt_tau,
                        alt_o,
                        alt_t_,
        ...
        if perform_update:
            ...
            self.alpha, self.beta, self.gamma, self.scaling, self.t_ = (
                alpha,
                beta,
                gamma,
                scaling,
    ...
    **kwargs,
    ...
    for i, gene in enumerate(var_names):
        ...
        if dm.recoverable:
            ...
            u0[ix], s0[ix], pval[ix], steady_u[ix], steady_s[ix] = (
                dm.u0,
                dm.s0,
                dm.pval_steady,
                dm.steady_u,
            ...
            std_u[ix], std_s[ix], likelihood[ix], varx[ix] = (
                dm.std_u,
                dm.std_s,
                dm.likelihood,
                dm.varx,
...
def align_dynamics(
    ...
    adata = data.copy() if copy else data
    alpha, beta, gamma, t_, scaling, mz = read_pars(
        adata, pars_names=["alpha", "beta", "gamma", "t_", "scaling", "alignment_scaling"]
    )
    T = adata.layers["fit_t"] if "fit_t" in adata.layers.keys() else np.zeros(adata.shape) * np.nan
    Tau = (
        adata.layers["fit_tau"]
    ...
    if mode == "align_total_time" and t_max is not False:
        T_max = np.max(T[:, idx] * (T[:, idx] < t_[idx]), axis=0) + np.max(
            (T[:, idx] - t_[idx]) * (T[:, idx] > t_[idx]), axis=0
        )
    ...
    if idx is None:
        alpha, beta, gamma, T, t_, Tau, Tau_ = (
            alpha / m,
            beta / m,
            gamma / m,
            T * m,
...
def latent_time(
    ...
    if root_key is None:
        keys = [
            key
            for key in ["root_cells", "starting_cells", "root_states_probs"]
    ...
    if root_key not in adata.uns.keys():
        ...
        if np.sum(idx_roots) > 0:
            ...
            logg.warn(
                "No root cells detected. Consider specifying root cells to improve latent time prediction."
            )
    else:
    ...
    VPT = velocity_pseudotime(adata, vkey, root_key=roots[0], end_key=fates[0], return_model=True)
    vpt = VPT.pseudotime
...
def rank_dynamical_genes(data, n_genes=100, groupby=None, copy=False):
    ...
    ll = get_divergence(
        vdata, mode="gene_likelihood", use_connectivities=True, clusters=adata.obs[groupby]
    )

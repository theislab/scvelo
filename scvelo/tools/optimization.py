...
def leastsq_generalized(
    x, y, x2, y2, res_std=None, res2_std=None, fit_offset=False, fit_offset2=False, perc=None
):
    ...
    if fit_offset and fit_offset2:
        for i in range(n_var):
            A = np.c_[
                np.vstack((np.c_[ones / res_std[i], zeros], np.c_[zeros, ones / res2_std[i]])),
                x[:, i],
            ]
...
def maximum_likelihood(Ms, Mu, Mus, Mss, fit_offset=False, fit_offset2=False):
    ...
    def sse(A, data, b):
        ...
        return np.log(
            sigma
        ).sum()  # np.log(np.sqrt(2*np.pi) * sigma).sum() + (.5 * (res/sigma[:, None])**2).sum()
    ...
    if fit_offset and fit_offset2:
        ...
        for i in range(n_var):
            ...
            offset[i], gamma[i] = minimize(
                lambda m: sse(
                    np.array([[1, -m[1], 0, 0], [1, m[1], 2, -2 * m[1]]]), data, b=np.array(m[0], 0)
                ),
                x0=(1e-4, 1),
                method="L-BFGS-B",
            ).x
        ...
        for i in range(n_var):
            ...
            offset_ss[i], gamma[i] = minimize(
                lambda m: sse(
                    np.array([[1, -m[1], 0, 0], [1, m[1], 2, -2 * m[1]]]), data, b=np.array(0, m[0])
                ),
                x0=(1e-4, 1),
                method="L-BFGS-B",
            ).x

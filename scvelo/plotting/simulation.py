...
def show_full_dynamics(
    ...
    if key != "true":
        ...
        if show_assignments is not None and show_assignments is not False:
            ...
            ax.plot(np.array([s, st]), np.array([u, ut]), color="grey", linewidth=0.1 * linewidth)
...
def simulation(
    ...
    **kwargs,
    ...
    for i, gs in enumerate(
        pl.GridSpec(1, ncols, pl.figure(None, (figsize[0] * ncols, figsize[1]), dpi=dpi))
    ):
        ...
        for j, key in enumerate(ykey):
            ...
            if key == "unspliced":
                ...
                ax.plot(
                    t, alpha, label="alpha", linestyle="--", color=colors[j], linewidth=linewidth
                )

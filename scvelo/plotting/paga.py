...
def paga(
    ...
    **kwargs,
    ...
    vkey = (
        [key for key in adata.layers.keys() if "velocity" in key and "_u" not in key]
        if vkey == "all"
        else vkey
    )
    layers, vkeys, colors = (
        make_unique_list(layer),
        make_unique_list(vkey),
        make_unique_list(color, allow_array=True),
    )
    ...
    if multikey is not None:
        ...
        if legend_loc is None:
            legend_loc = "none"
        kwargs["legend_loc"] = (
            "none" if legend_loc == "on data" else legend_loc
        )  # let paga handle 'on data'
...
def _compute_pos(
    ...
    if layout == "fa":
        init_coords = (
            np.random.random((adjacency_solid.shape[0], 2)) if init_pos is None else init_pos.copy()
        )
        ...
        if len(pos) < adjacency_solid.shape[0]:
            raise ValueError(
                "This is a forest and not a single tree. Try another `layout`, e.g., {'fr'}."
            )
    else:
        # igraph layouts
    ...
    **scatter_kwargs,
    ...
    if pos is not None:
        if isinstance(pos, str):
            ...
            pos = (
                np.stack(
                    [np.median(adata.obsm[pos][adata.obs[groups_key] == c], axis=0) for c in cats]
                )
        ...
        if isinstance(init_pos, str):
            ...
            init_pos = (
                np.stack(
                    [
                        np.median(adata.obsm[init_pos][adata.obs[groups_key] == c], axis=0)
                        for c in cats
                    ]
                )
...
def _paga_graph(
    ...
    if (
        ...
        raise ValueError(
            f"Provide a list of group labels for the PAGA groups {adata.uns['paga']['groups']}, not {node_labels}."
        )
    groups_key = adata.uns["paga"]["groups"]
    ...
    if n_components > 1 and single_component:
        ...
        logg.info(
            f"Restricting graph to largest connected component by dropping categories\n{cats_dropped}"
        )
        nx_g_solid = nx.Graph(adjacency_solid)
        if dashed_edges is not None:
    ...
    if dashed_edges is not None:
        ...
        nx.draw_networkx_edges(
            nx_g_dashed, pos, ax=ax, width=widths, edge_color="grey", style="dashed", alpha=0.5
        )
    ...
    # else pie chart plot
        ...
        for count, (n, box) in enumerate(zip(nx_g_solid.nodes(), bboxes)):
            ...
            if xa < 0:
                xa = 0
            pie_axs.append(
                pl.axes([xa, ya, pie_size * ax_len_x, pie_size * ax_len_y], frameon=False)
            )

...
def make_dense(X):
    XA = (
        X.A
        if issparse(X) and X.ndim == 2
        else X.A1
        if issparse(X) or isinstance(X, np.matrix)
        else X
    )
    return np.array(XA)
...
def is_list_of_str(key, max_len=None):
    if max_len is not None:
        return (
            isinstance(key, (list, tuple, np.record, np.ndarray))
            and len(key) < max_len
            and all(isinstance(item, str) for item in key)
        )
    else:
        return isinstance(key, (list, tuple, np.record)) and all(
...
def is_list_of_int(lst):
    return isinstance(lst, (list, tuple, np.ndarray, np.record)) and all(
        isinstance(item, (int, np.integer)) for item in lst
    )
...
def gets_vals_from_color_gradients(adata, color=None, **scatter_kwargs):
    ...
    if "s" not in scatter_kwargs:
        scatter_kwargs["s"] = (
            default_size(adata) if scatter_kwargs["size"] is None else scatter_kwargs["size"]
        )
...
def update_axes(
    ax, xlim=None, ylim=None, fontsize=None, is_embedding=False, frameon=None, figsize=None
):
    if xlim is not None:
        ax.set_xlim(xlim)
    if ylim is not None:
    ...
    if isinstance(frameon, str) and frameon == "artist":
        ...
        if isinstance(frameon, str) and frameon != "full":
            ...
            ax.tick_params(
                which="both", bottom=bf, left=lf, top=tf, right=rf, labelbottom=bf, labelleft=lf
            )
...
def set_label(xlabel, ylabel, fontsize=None, basis=None, ax=None, **kwargs):
    labels, labels_new = (
        np.array(["Ms", "Mu", "X"]),
        np.array(["spliced", "unspliced", "expression"]),
    )
    if xlabel in labels:
...
def get_colors(adata, c):
    if is_color_like(c):
        ...
        cluster_ix = (
            ...
            else adata.obs[c].cat.codes.values
        )
        return np.array(
            [
...
def interpret_colorkey(adata, c=None, layer=None, perc=None, use_raw=None):
    ...
    if is_categorical(adata, c):
        ...
        if c in adata.obs.keys():  # color by observation key
            ...
            if layer in adata.layers.keys():
                if perc is None and any(
                    ...
                    perc = [
                        1,
                        99,
                    ]  # clip values to ignore extreme outliers since these layers are not logarithmized
                ...
                if adata.raw is None and use_raw:
                    raise ValueError(
                        "`use_raw` is set to True but AnnData object does not have raw. Please check."
                    )
            ...
            var = adata.var[
                [key for key in adata.var.keys() if not isinstance(adata.var[key][0], str)]
            ]
            c = var.astype(np.float32).eval(c)
        elif np.any([obs_key in c for obs_key in adata.obs.keys()]):
            obs = adata.obs[
                [key for key in adata.obs.keys() if not isinstance(adata.obs[key][0], str)]
            ]
...
def savefig_or_show(writekey=None, show=None, dpi=None, ext=None, save=None):
    ...
    if save:
        ...
        try:
            filename = (
                settings.figdir + f"{settings.plot_prefix}{writekey}{settings.plot_suffix}.{ext}"
            )
            ...
            filename = (
                settings.figdir + f"{settings.plot_prefix}{writekey}{settings.plot_suffix}.png"
            )
            pl.savefig(filename, dpi=dpi, bbox_inches="tight")
...
def plot_linfit(
    x, y, add_linfit=True, add_legend=True, color=None, linewidth=None, fontsize=None, ax=None
):
    if ax is None:
        ax = pl.gca()
    ...
    color = (
        add_linfit if isinstance(add_linfit, str) else color if isinstance(color, str) else "grey"
    )
    ...
    if add_legend:
        ax.text(
            0.05,
            0.95,
            r"$\rho = $" + f"{np.round(pearsonr(x, y)[0], 2)}",
            ha="left",
            va="top",
            fontsize=fontsize,
...
def plot_polyfit(
    x, y, add_polyfit=True, add_legend=True, color=None, linewidth=None, fontsize=None, ax=None
):
    if ax is None:
        ax = pl.gca()
    idx_valid = ~np.isnan(x + y)
    ...
    if isinstance(add_polyfit, str) and "intercept" in add_polyfit:
        ...
        add_polyfit = (
            [a for a in str_list if "intercept" not in a][0] if "," in add_polyfit else None
        )
    color = (
        add_polyfit if isinstance(add_polyfit, str) else color if isinstance(color, str) else "grey"
    )
...
def plot_vlines(adata, basis, vkey, xkey, linewidth=1, linecolor=None, ax=None):
    ...
    for i, fit in enumerate(fits):
        ...
        gamma = (
            adata[:, basis].var[f"{fit}_gamma"].values if f"{fit}_gamma" in adata.var.keys() else 1
        )
        beta = adata[:, basis].var[f"{fit}_beta"].values if f"{fit}_beta" in adata.var.keys() else 1
        offset = (
            adata[:, basis].var[f"{fit}_offset"].values
            if f"{fit}_offset" in adata.var.keys()
            else 0
        )
        (line,) = ax.plot(
            xnew,
            gamma / beta * xnew + offset / beta,
            linestyle=linestyle,
            linewidth=linewidth,
            c=linecolor[i]
...
def plot_velocity_fits(
    ...
    lines, fits = plot_vlines(
        adata, basis, vkey, "spliced" if use_raw else "Ms", linewidth, linecolor, ax=ax
    )
    ...
    if "fit_alpha" in adata.var.keys() and (vkey is None or "dynamics" in vkey):
        line, fit = show_full_dynamics(
            adata, basis, "fit", use_raw, linewidth, show_assignments=show_assignments, ax=ax
        )
        fits.append(fit)
        lines.append(line)
...
def plot_density(
    ...
    if ax is None:
        ax = pl.gca()
    color = (
        add_density if isinstance(add_density, str) else color if isinstance(color, str) else "grey"
    )
...
def plot_rug(x, height=0.03, color=None, ax=None, **kwargs):
    ...
    line_segs = np.column_stack([np.repeat(x, 2), np.tile([0, height], len(x))]).reshape(
        [len(x), 2, 2]
    )
...
def hist(
    ...
    **kwargs,
    ...
    set_label(
        ...
        ylabel if xlabel is not None else "",
        fontsize=fontsize,
        ax=ax,
    )

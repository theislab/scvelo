    ...
    **kwargs,
    ...
    if isinstance(groupby, str) and groupby in adata.obs.keys():
        ...
        if groups is None:
            ...
            idx = np.array(
                [any([g in group for g in groups]) for group in adata.obs[groupby].cat.categories]
            )
        ...
        var_names = (
            ...
            else [var for var in var_names if var in adata.var_names]
        )
    else:
    ...
    (skey, ukey) = (
        ("spliced", "unspliced") if use_raw or "Ms" not in adata.layers.keys() else ("Ms", "Mu")
    )
    ...
    fits = [fit for fit in fits if fit == "dynamics" or f"{fit}_gamma" in adata.var.keys()] + [
        "dynamics"
    ]
    ...
    ax = (
        pl.figure(figsize=(figsize[0] * ncols / 2, figsize[1] * nrows / 2), dpi=dpi)
        if ax is None
        else ax
    )
    ...
    size = (
        default_size(adata) / 2 if size is None else size
    )  # since fontsize is halved in width and height
    ...
    for v, var in enumerate(var_names):
        ...
        if mode == "stochastic":
            ...
            for fit in stochastic_fits:
                gamma = (
                    _adata.var[f"{fit}_gamma"].values if f"{fit}_gamma" in adata.var.keys() else 1
                )
                beta = _adata.var[f"{fit}_beta"].values if f"{fit}_beta" in adata.var.keys() else 1
                offset2 = (
                    _adata.var[f"{fit}_offset2"].values

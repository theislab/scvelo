...
def scatter(
    ...
    **kwargs,
    ...
    if multikey is not None:
        ...
        gspec = pl.GridSpec(
            nrows, ncols, fig, hspace=0.3 if hspace is None else hspace, wspace=wspace
        )
        ...
        for i, gs in enumerate(gspec):
            if i < len(multikey):
                multi_kwargs = {
                    "groups": groups[i * (len(groups) > i)] if is_list_of_list(groups) else groups
                }
                for (
                    key
                ) in (
                    mkeys
                ):  # multi_kwargs[key] = key[i] if is multikey (list with more than 1 element) else key
                    multi_kwargs[key] = eval(
                        "{0}[i * (len({0}) > i)] if is_list({0}) else {0}".format(key)
                    )
                ax.append(
                    scatter(adata, ax=pl.subplot(gs), **multi_kwargs, **scatter_kwargs, **kwargs)
                )

        if not frameon and isinstance(
            ylabel, str
        ):  # later makes this a label on the figure rather than axis.
        ...
        if any([isinstance(key, str) and "," in key for key in [y, layer]]):
            ...
            if multikey is not None:
                ...
                if legend_loc and legend_loc != "none":
                    multikey = [
                        key.replace("Mu", "unspliced").replace("Ms", "spliced") for key in multikey
                    ]
            ...
            c_colors = {
                cat: col
                for (cat, col) in zip(adata.obs[color].cat.categories, adata.uns[f"{color}_colors"])
            }
            mkwargs.pop("color")
            ax = scatter(
                adata, color="grey", ax=ax, **mkwargs, **get_kwargs(scatter_kwargs, {"alpha": 0.05})
            )  # background
            ax = scatter(
                adata, color=color, ax=ax, **mkwargs, **get_kwargs(scatter_kwargs, {"s": 0})
            )  # set legend
            sorted_idx = np.argsort(vals, 1)[:, ::-1][:, :2]
            for id0 in range(len(names)):
                for id1 in range(id0 + 1, len(names)):
                    cmap = rgb_custom_colormap(
                        [c_colors[names[id0]], "white", c_colors[names[id1]]], alpha=[1, 0, 1]
                    )
        ...
        # actual scatter plot
            ...
            if basis in adata.var_names:
                ...
                if x is None and y is None:
                    x, y = (
                        default_xkey(adata, use_raw=use_raw),
                        default_ykey(adata, use_raw=use_raw),
                    )
                ...
                if use_raw and perc is not None:
                    ax.set_xlim(
                        right=np.percentile(x, 99.9 if not isinstance(perc, int) else perc) * 1.05
                    )
                    ax.set_ylim(
                        top=np.percentile(y, 99.9 if not isinstance(perc, int) else perc) * 1.05
                    )
            ...
            # embedding: set x and y to embedding coordinates
                ...
                is_timeseries = y in var_names and x in list(adata.obs.keys()) + list(
                    adata.layers.keys()
                )
                if xlabel is None:
                    xlabel = x
                    ...
                    if x in var_names and y in var_names:
                        ...
                        var = adata.var[
                            [
                                ...
                                if not isinstance(adata.var[key][0], str)
                            ]
                        ]
                        ...
                        obs = adata.obs[
                            [
                                ...
                                if not isinstance(adata.obs[key][0], str)
                            ]
                        ]
                        ...
                        raise ValueError(
                            "x or y is invalid! pass valid observation annotation or a gene name"
                        )
            ...
            if n_convolve is not None:
                y[np.argsort(x)] = np.convolve(
                    y[np.argsort(x)], np.ones(n_convolve) / n_convolve, mode="same"
                )
            ...
            if basis in adata.var_names and isinstance(color, str) and color in adata.layers.keys():
                c = interpret_colorkey(
                    adata, basis, color, perc, use_raw
                )  # phase portrait: color=basis, layer=color
            else:
                c = interpret_colorkey(
                    adata, color, layer, perc, use_raw
            ...
            if c is not None and not isinstance(c, str) and not isinstance(c[0], str):
                ...
                if smooth and len(c) == adata.n_obs:
                    c = get_connectivities(
                        adata, n_neighbors=(None if isinstance(smooth, bool) else smooth)
                    ).dot(c)
                ...
                if rescale_color is not None:
                    try:
                        if len(rescale_color) != 2:
                            raise ValueError(
                                "rescale_color has to be a tuple with two values, e.g. [0,1]."
                            )
            ...
            if (
                ...
                _ = [
                    ...
                    for key in ["groups", "add_linfit", "add_polyfit", "add_density"]
                ]
                ax = scatter(
                    adata,
                    x=x,
            ...
            if not isinstance(c, str) and len(c) == len(x):
                ...
                if sort_order and not is_categorical(adata, color):
                    ...
                    order = np.random.choice(
                        np.arange(0, len(x)), len(x), replace=False, p=counts / np.sum(counts)
                    )
            ...
            if isinstance(add_outline, (list, tuple, np.ndarray, int, np.int_, str)) or add_outline:
                ...
                if idx is not None and np.sum(idx) > 0:  # if anything to be outlined
                    ...
                    x, y, c = (
                        x[idx],
                        y[idx],
                        c[idx] if not isinstance(c, str) and len(c) == adata.n_obs else c,
                    )
                    if isinstance(kwargs["s"], np.ndarray):
                        kwargs["s"] = np.array(kwargs["s"])[idx]
                    if (
                        isinstance(c, np.ndarray)
                        and not isinstance(c[0], str)
                        and "vmid" not in kwargs
                    ):
            ...
            if is_categorical(adata, color) and len(scatter_array) == adata.n_obs:
                ...
                if not (add_outline is None or groups_to_bool(adata, add_outline, color) is None):
                    groups = add_outline
                set_legend(
                    adata,
                    ax,
            ...
            if add_linfit:
                ...
                plot_linfit(
                    x, y, add_linfit, legend_loc != "none", linecolor, linewidth, fontsize, ax=ax
                )
            ...
            if add_polyfit:
                ...
                plot_polyfit(
                    x, y, add_polyfit, legend_loc != "none", linecolor, linewidth, fontsize, ax=ax
                )

            if add_rug:
                rug_color = interpret_colorkey(
                    adata, add_rug if isinstance(add_rug, str) else color
                )
            ...
            if (
                colorbar is not False
                and not isinstance(c, str)
                and not is_categorical(adata, color)
            ):
                set_colorbar(
                    smp, ax=ax, labelsize=fontsize * 0.75 if fontsize is not None else None
                )
...
def draw_graph(adata, layout=None, **kwargs):
    ...
    if f"X_{basis}" not in adata.obsm_keys():
        raise ValueError(
            f"Did not find draw_graph_{layout} in adata.obs. Did you compute layout {layout}?"
        )

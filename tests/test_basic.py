...
def test_neighbors():
    ...
    assert np.all(
        np.round(adata.obsp["distances"][0].data, 2)
        == np.round(adata_.obsp["distances"][0].data, 2)
    )
...
def test_pipeline():
    ...
    with scv.GridSpec() as pl:
        ...
        pl.velocity_embedding_grid(adata, scale=0.5, density=0.5, c="latent_time", cmap="gnuplot")
